...
def fetch_search_console_data(site_url: str) -> dict | None:
    ...
    try:
        ...
        # 「あと一歩」キーワード抽出。立ち上げ初期の小規模サイトでも拾えるよう
        # 表示10回以上・順位4〜30位（1ページ目手前〜数ページ）まで広めに見る
        opportunity_keywords = sorted(
            [q for q in queries if 4 <= q["position"] <= 30 and q["impressions"] >= 10],
...
【最重要：サイトの現状を踏まえた戦略】
このブログは開設から日が浅く、ドメインの力（被リンク・運営歴）がまだ弱い小規模サイトです。
そのため「新NISA」「副業 おすすめ」「高配当株」のような検索数が多い王道キーワード（ビッグキーワード）では、
大手金融メディア・大手企業に絶対に勝てません。提案してはいけません。

代わりに、以下の条件を満たす「ロングテール（複合・低競合）キーワード」だけを狙ってください。
- 3語以上の組み合わせ（例：「新NISA 主婦 月1万円」「コワーキング 地方 安い 個室」）
- 「対象者・状況・悩み・否定形・地域」などの修飾語を必ず含める
  例の修飾語: 初心者 / 主婦 / 40代 / 在宅 / スマホだけ / 失敗 / やめとけ / 怪しい / 比較 / 違い / 田舎 / 一人
- 検索する人が少なくても「悩みが具体的で・競合が弱く・買う前の人」が読むキーワードを優先
- 既存の流入KW（あれば上のSEO機会KW）を伸ばす方向のテーマも歓迎

【良い例】「ConoHa WING 評判 初心者 つまずきやすい点」（低競合・具体的）
【悪い例】「WordPressブログの始め方」（激戦・新規サイトでは勝てない）
...
target_keyword は必ず3語以上の複合キーワードにすること:
